...
let channels = [
    ( "keyboard", PushBufferedChannel { delay = None; typ = T_String; handler = keyboard_handle; initial_buffer = String_rv ""});
    ( "second", PushChannel { delay = Some 1.0; typ = T_Unit; handler = (fun () -> Unit_rv)});
    ( "minute", PushChannel { delay = Some 60.0; typ = T_Unit; handler = (fun () -> Unit_rv)});
    ( "system_time", BufferedChannel { delay = Some 1.0; typ = T_Int; handler = system_time_handle; initial_buffer = (Int_rv 0) });
